...
function getOverAllPercentage(report: ReportObject) {
  const totalFound =
    report.total.lines.found + report.total.functions.found + report.total.branches.found;
  const totalCovered =
    report.total.lines.covered + report.total.functions.covered + report.total.branches.covered;
...
`;
  markdown = markdown.replace(rexPercentage, percentage).replace(rexColor, color);
