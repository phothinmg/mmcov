...
const fileRows = (fileObj: FileObject) => {
  ...
  html = html.replace(indexRegexp.fileLinePercentage, `${fileObj.lines.percentage.toFixed(2)}%`);
  html = html.replace(indexRegexp.fileLineInfo, `${fileObj.lines.hit}/${fileObj.lines.found}`);
