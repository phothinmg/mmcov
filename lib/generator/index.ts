...
class ReportGenerator {
  ...
  private isLcov(text: string) {
    ...
    const hasValidPrefixes = /^(TN:|SF:|FN:|DA:|BRDA:|LF:|LH:|BRF:|BRH:|FNF:|FNH:)/m.test(text);
  ...
  private readLcov() {
    ...
    if (this._sources && this._sources.length > 0) {
      const source_paths = this._sources.map((source) => `SF:${source.replace(/^\.\//, "")}/`);
  ...
  private async generateLcovSingleFile(input: string) {
    ...
    for (const line of lines) {
      ...
      // 1. functions -> details -> executionCount
        ...
        const func = result.functions.details.find((f) => f.name === functionName);
        if (func) {
      ...
      // 3. branches -> details
        ...
        const taken = parts[3] === "-" ? 0 : Number.parseInt(parts[3] || "0", 10);
    ...
    if (result.functions.found > 0) {
      result.functions.percentage = (result.functions.hit / result.functions.found) * 100;
    }
    if (result.branches.found > 0) {
      result.branches.percentage = (result.branches.hit / result.branches.found) * 100;
    }
    ...
    if (result.file.fileExtension) {
      ...
      result.file.highlightedCode = await shikiHL(code, result.file.fileExtension);
    }
  ...
  async generate() {
    ...
    if (this._report.total.lines.found > 0) {
      this._report.total.lines.percentage =
        (this._report.total.lines.covered / this._report.total.lines.found) * 100;
    }
    if (this._report.total.functions.found > 0) {
      this._report.total.functions.percentage =
        (this._report.total.functions.covered / this._report.total.functions.found) * 100;
    }
    if (this._report.total.branches.found > 0) {
      this._report.total.branches.percentage =
        (this._report.total.branches.covered / this._report.total.branches.found) * 100;
    }
    return this._report;
  }
}
