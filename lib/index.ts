    ...
    while (i < argvs.length) {
      ...
      if (i === 0 && !arg.startsWith("-")) {
        ...
        switch (arg) {
          ...
          case "--mmdocs":
            if (
              (argvs[i + 1] && argvs[i + 1] !== "" && (argvs[i + 1] as string).startsWith("-")) ||
              argvs[i + 1] === "" ||
              argvs[i + 1] === undefined
            ) {
              opts.mmdocs = true;
